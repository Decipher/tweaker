 ...
 */
class tweaker_ui extends ctools_export_ui {
  //var $plugin;
  //var $name;
  //var $options = array();
  //
  ...
  function edit_form(&$form, &$form_state) {
    ...
    $form['hook'] = array(
      ...
      '#type' => 'select',
      '#options' => array(),
      '#default_value' => $item->hook,
    );
    foreach (array_keys($info['hooks']) as $hook) {
      $form['hook']['#options'][$hook] = "hook_{$hook}()";
    }

    //$form['conditions']['require_all'] = array(
    //  '#title' => t('Require all conditions'),
    //  '#type' => 'checkbox',
    //  '#default_value' => isset($item->conditions['require_all']) ? $item->conditions['require_all'] : 0,
    //);
    ...
    foreach ($item->conditions as $delta => $condition) {
      if (is_numeric($delta) && isset($info['conditions'][$condition['type']])) {
        $form['conditions'][$delta] = array(
          '#title' => $info['conditions'][$condition['type']]['title'],
          '#type' => 'fieldset',
        );
        $form['conditions'][$delta]['type'] = array(
          '#type' => 'value',
          '#value' => $condition['type'],
        );

        // Include callback file.
        if (isset($info['conditions'][$condition['type']]['file']) && file_exists($file = $info['conditions'][$condition['type']]['file'])) {
          require_once $file;
        }

        if (function_exists($function = $info['conditions'][$condition['type']]['callbacks']['settings form'])) {
          $function($form['conditions'][$delta], $form_state, $condition);
        }
      }
    }
